...
static struct rdb_txt_s rdb_txt;
...
static int _valid_data(XML_CONFIG *config) {
	...
	if (!rdb_txt.common.record) {
		LOG_ERR("invalid record at line #%d, no record attribute", config->line);
		return -1;
	}

	if (!rdb_txt.data) {
		LOG_ERR("invalid record at line #%d, missing data for TXT record", config->line);
		return -1;
	}
...
static int _make_rr(XML_CONFIG *config) {
	...
	if ((rr = rdb_prepare_rr(&(rdb_txt.common), config)) == NULL) {
	...
	rtv = dns_mk_txt_data(rdb_txt.data, rr);
	if (rtv == -1) {
...
static int _text(char *text, XML_CONFIG *config) {
	if (strcasecmp(config->xpath, "/nnodes/zones/zone/txt/txt:data") == 0)
		rdb_txt.data = strdup(text);

	return 0;
}

static int _tag_attr(char *tagname, char *attrname, char *attrvalue, XML_CONFIG *config) {
	if (strcasecmp(config->xpath, "/nnodes/zones/zone/txt") == 0)
		return rdb_tag_attr(&(rdb_txt.common), attrname, attrvalue, config);
	else if (strcasecmp(config->xpath, "/nnodes/zones/zone/txt/txt:data") == 0)
		return rdb_data_tag_attr(&(rdb_txt.common), attrname, attrvalue, config);
...
static int _tag_close(char *tagname, XML_CONFIG *config) {
	if (strcasecmp(config->xpath, "/nnodes/zones/zone/txt/txt:data") == 0) {
		...
		if (rdb_txt.data)
			mm_free0((void*) &rdb_txt.data);

		rdb_data_tag_close(&(rdb_txt.common));
	} else if (strcasecmp(config->xpath, "/nnodes/zones/zone/txt") == 0)
		rdb_tag_close(&(rdb_txt.common));

	return 0;
}

void rdb_txt_add_xc(HCODE *xch) {
	XML_CALLBACKS xc;
	...
	hl_hash_put(xch, "/nnodes/zones/zone/txt", &xc, sizeof(XML_CALLBACKS));
}
